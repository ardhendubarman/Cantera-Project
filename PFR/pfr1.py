...
ign_delay=t2[inflexion]-t2[0]
ignition_delay.append(ign_delay)
print ('ignition delay time= %f'%(ign_delay))
print (t2[inflexion])
plt.figure()
plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))
plt.plot(t2, states2.T, 'k', label='Reactor Chain')
plt.plot(t2[inflexion], states2.T[inflexion], 'ro', label='maximum inflection')
plt.plot(t2[inflexion2], states2.T[inflexion2], 'bo',label='minimum inflection')
plt.vlines(t2[inflexion], -5, 9, linewidth=2, alpha=0.3)
plt.xlabel('$t$ [s]')
plt.ylabel('$T$ [K]')
plt.legend(loc=0)
plt.show()
